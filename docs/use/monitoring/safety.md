...
The **physical emergency stop** must be implemented independently of the browser and network and
must stop hazardous motion according to the validated safety design. It remains available when the
software path does not. It is the one you use in an emergency.

Every session, before the robot moves: **locate the physical stop and confirm that its required
inspection or test has been completed using the approved site procedure.**
...
8. Physical emergency stop located and its required check recorded
...
!!! warning
    In the current simulated docking path, scanning, alignment and final approach publish motion
    commands directly rather than through Nav2 obstacle handling. Do not assume software obstacle
    checking will stop the robot during this manoeuvre.
...
The Domain Expert is responsible for following the approved operating procedure and maintaining
the immediate operating boundary during use.
